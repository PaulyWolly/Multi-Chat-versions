...
app.get('/api/chat', (req, res) => {
    logSSEConnection(req, 'INITIATED');

    res.setHeader('Content-Type', 'text/event-stream');
    res.setHeader('Cache-Control', 'no-cache');
    res.setHeader('Connection', 'keep-alive');

    logSSEConnection(req, 'ESTABLISHED');

    req.on('close', () => {
        logSSEConnection(req, 'CLOSED');
        if (req.headers['last-event-id']) {
            console.log('Client disconnected, cleaning up:', req.headers['last-event-id']);
        }
    });
});
...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        // Check if this is a combined info + images request
        const isInfoWithImages = message.toLowerCase().match(/tell me about .+ and provide images/i) ||
                               message.toLowerCase().match(/what (?:is|are) .+ and (?:show|provide) images/i) ||
                               message.toLowerCase().match(/describe .+ and (?:show|include|provide) images/i);
...
app.get('/api/google-image-search', async (req, res) => {
    try {
        ...
        const response = await fetch(`${searchUrl}?key=${process.env.GOOGLE_API_KEY}&cx=${process.env.GOOGLE_SEARCH_ENGINE_ID}&q=${encodeURIComponent(query)}&searchType=image`);
...
// =====================================================
// END OF server.js FILE 19.5-R-P-J-YT
// =====================================================
